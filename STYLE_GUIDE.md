...
- Three different quantities are spelled `tol` on the public surface, and the
  distinction is not visible at a call site: `analyze_symmetry(...; tol)` /
  `SCEBasis(...; tol)` is the backend's symprec, a **Cartesian distance in Å**;
  `build_neighbor_list(...; tol)` and `SCEBasis(...; tie_tol)` are the
  **relative** same-distance band; `AdaptiveRidge(...; tol)` is a **relative**
  convergence threshold. Never compare one against a quantity carrying another's
  unit, and divide a length by a length before comparing it to a dimensionless
  one (`_frac_bound`, `_fractional_matrix_atol` and `_orthogonality_atol` in
  `symmetry/backend.jl` are the worked examples).
