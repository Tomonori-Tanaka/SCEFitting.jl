...
- **Q6 `cutoff_star` の精密化。** 現状 1 枚のペア表を N−1 本すべてに適用（max 型の制約で
  置換不変）。「1 本目は近く 2 本目は遠く」は**サイトが置換で入れ替わるので定義できない**
  （ラベルは多重集合）。置換不変で意味のある精密化は「スポーク長の**総和**」や
  「クラスター直径」の上限。スコープ外だが、docs に「スポーク別半径は対称性上定義できない、
  総和型なら可能」と 1 行残す。
