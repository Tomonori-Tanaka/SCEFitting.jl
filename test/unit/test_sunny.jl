...
@testset "Sunny export (core conversion math)" begin
    ...
    @testset "_classify_salc" begin
        sd(ls...) = SCEFitting.spin_decors(collect(Int, ls))
        ...
        @test _classify_salc([SCEFitting.SiteDecor(; spin = 1),
                              SCEFitting.SiteDecor(; spin = 1, disp = (0, 1))]) ===
              :unsupported
        @test _classify_salc([SCEFitting.SiteDecor(; disp = (0, 1))]) === :unsupported
    end
    ...
    @testset "primitive unfold: two sublattices, no inversion" begin
        ...
        # [Backported from SLCE.jl b0593ef.]
        lat = Lattice([8.0 0 0; 0 8.0 0; 0 0 10.0])
        ...
        basis = SCEBasis(cr, sg, salcs,
                         BasisSpec(; nbody = 2, cutoff = 3.6, lmax = [1, 1],
                                   isotropy = false))
        rng = MersenneTwister(11)
        model = SCEPredictor(basis, 0.25, randn(rng, n_salcs(basis)),
                             basis.salc_basis.keys)
