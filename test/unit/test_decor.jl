...
@testset "decoration labels" begin
    ...
    @testset "Slot is the axis -> (site, factor) map" begin
        # the pure-spin identity: axis i sits on site i and carries SPIN l = ls[i]
        ls = [2, 1, 3]
        slots = spin_slots(ls)
        @test length(slots) == length(ls)
        @test [s.site for s in slots] == collect(eachindex(ls))
        @test all(s.factor.channel == SPIN && s.factor.k == 0 for s in slots)
        @test [s.factor.l for s in slots] == ls
        # a site may host two slots of different channels (what the map generalizes)
