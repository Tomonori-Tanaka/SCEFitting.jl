# Displacement-channel solid harmonics: 4π-free (Racah-type) normalization and
# polynomial regularity at u = 0. Values only — this package carries no
# displacement gradient (see the module docstring).

using SCEFitting: Harmonics, SolidHarmonics
using StaticArrays
...
@testset "SolidHarmonics" begin
    ...
    @testset "u = 0 exactness" begin
        z3 = [0.0, 0.0, 0.0]
        vals = SolidHarmonics.solid_harmonics(5, z3)
        @test vals[1] == 1.0
        for l = 1:5, m = -l:l
    ...
    @testset "single-(l,m) accessors agree with the batch evaluator" begin
        u = randn(rng, 3) * 1.3
        vals = SolidHarmonics.solid_harmonics(4, u)
        for l = 0:4, m = -l:l
            idx = SolidHarmonics.solid_harmonic_index(l, m)
