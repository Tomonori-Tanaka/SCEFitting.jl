...
@testset "SCEDataset slicing / vcat" begin
    ...
    @testset "a basis with no columns cannot carry training data" begin
        ...
        # every configuration. [Backported from SLCE.jl 92df67c.]
        empty_b = SCEBasis(crystal, BasisSpec(; nbody = 2, cutoff = 0.05, lmax = [2],
                                              isotropy = true))
        @test n_salcs(empty_b) == 0
        @test_throws ArgumentError SCEDataset(empty_b, configs, energies)
        err = try
            SCEDataset(empty_b, configs, energies)
        catch e
            e
        end
        @test occursin("no SALC columns", err.msg)
        @test occursin("image of ITSELF", err.msg)
        @test_throws ArgumentError SCEDataset(empty_b, configs, energies, torques)
    end
end
