...
# m_i(e) — the moment channel's counterpart of `SCEBasis`.
...
# (measured upstream: 3 of 6 orderings halved the closed-star column against the
# prototype's 6.0 geometric oracle), silently breaking cross-orbit coefficient
# comparability. So the enumeration here first finds each star once (per translation
# class), then expands EVERY class to all `N!` re-anchored orderings — identical
# members to what `candidate_clusters` would emit for the clusters it also admits.
...
function _design_moment(mb::MomentBasis, configs::Vector{Matrix{Float64}},
    ...
    # sit together at the end. `:dynamic` would NOT help — it is the same contiguous
    # per-thread chunking, differing only in thread affinity; `:greedy` is the one that
    # hands out iterations individually. Each `j` writes only column `j` and reduces
    # serially, so no value depends on the schedule.
...
function _moment_resolvability(mb::MomentBasis, rtol::Float64)
    ...
    for (j, s) in enumerate(sal), mem in s.members, t in mem.terms
        ...
        # of it. This is the BACKSTOP, not the primary door: `_pointed_star_candidates`
        # refuses a star whose sites repeat a reference-cell atom, which is where both
        # collisions below are actually stopped for the enumeration this package ships.
        # What reaches here is a candidate source that does not apply that rule. Note
        # `admit`'s spoke test cannot stand in for either: `_dmin2_matrix` leaves the
        # diagonal at `Inf` (the list drops `i == j`), so `dmin2_star[a,a]` is
        # identically true. Do not remove the `mem.atoms[mark_site]` term.
