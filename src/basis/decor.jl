...
occupation channel is additive, but constructing an `OCC` [`SiteFactor`](@ref) is
an error in this version.

Public but unexported (the name would shadow `Base.Channel`); reach it as
`SCEFitting.Channel`, `SCEFitting.SPIN`, `SCEFitting.DISP`, `SCEFitting.OCC`.
"""
...
# (measured: three environments of this package, three different values for the
