...
edge admissibility — must use the same band, or a degenerate shell is admitted by
one and split by the other. Reading the field is the only way those stay in step
when a caller passes a non-default `tol`. [Backported from SLCE.jl 72a9cc1.]
"""
struct NeighborList
    cutoff::Float64
