...
# satisfies by construction. [Factoring from SLCE.jl 6960276; behavior identical.]
    ...
    begin
        ...
        for s0 in order
            ...
            for s in orbit_sigs
                ...
                # (no cost) or it is violated (must be loud). Violation is REACHABLE: on a
                # slab supercell whose minimum-image ties are broken arbitrarily, the
                # candidate set need not be group-closed, and the silent skip here built
                # short orbits whose SALCs broke the space group by ~4 meV (MnTe(0001)
                # 3×3 slab, 54 spurious Lf ≠ 0 SALCs — found 2026-08-12).
                # [Backported from SLCE.jl d4660a7.]
