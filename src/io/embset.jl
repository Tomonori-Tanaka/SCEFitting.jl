...
function read_embset(path::AbstractString; n_atoms::Union{Integer,Nothing} = nothing,
    ...
    return SpinDatum[SpinDatum(energies[c], moments[c], fields[c];
...
"""
    read_embset_pair(mw_path, mint_path; n_atoms = nothing, zero_moment_atol = 1e-10,
                     constraint_mode = nothing, constraint_axes = nothing,
...
The result is [`SpinDatum`](@ref)s built from the `mw_path` observables with
...
function read_embset_pair(mw_path::AbstractString, mint_path::AbstractString;
                          ...
                          constraint_axes::Union{
                              Nothing,AbstractMatrix{<:Real},
    ...
    data = SpinDatum[SpinDatum(e_mw[c], m_mw[c], b_mw[c];
                               zero_moment_atol = zero_moment_atol,
