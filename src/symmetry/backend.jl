...
_tclose(A::SMatrix{3,3,Float64,9}, a::SVector{3,Float64},
...
function _find_op(ops::Vector{SymOp}, bykey::Dict{SMatrix{3,3,Int,9},Vector{Int}},
    ...
    for i in idx
        _tclose(A, ops[i].translation_frac, t, tol) && return i
    end
    return 0
end
...
function _validate_ops(ops::Vector{SymOp}, lattice::Lattice, tol::Real)
    ...
    for (_, idx) in bykey, a in eachindex(idx), b in (a + 1):length(idx)
        _tclose(A, ops[idx[a]].translation_frac,
                ops[idx[b]].translation_frac, tol) &&
            throw(ArgumentError(
