...
- **Two public documentation pages still described the pre-refusal behaviour.**
  `guide/moment.md` told the reader to "expect a cell that resolved at three bodies to
  refuse at four", and `theory/moment.md` said a basis with two environment factors on
  one reference-cell atom "is refused as unclassifiable" — both true before the
  enumeration-level rule landed, neither true after. Rewritten to the shipped
  behaviour, with the Clebsch–Gordan argument for why such a member was never an
  N-body function.
...
- **`analyze_symmetry(...; tol)` is the backend's symprec — a distance in Å — and the
  ...
  `_tclose` folded the FRACTIONAL difference and compared its Euclidean length to `tol`
